...
.set_rival_facing_direction
	...
	ld hl, .StarterTable
	call Route22GetRivalTrainerNoByStarterScript
; (The starter-permadeath pre-battle bookkeeping that used to live here moved
; to the burned-lab Mewtwo ambush, scripts/PokemonMansion1F.asm -- per user
; direction the starter can only die in THAT fight.)
...
; (The post-battle starter-death/ashes check that used to live here moved to
; the burned-lab Mewtwo ambush, scripts/PokemonMansion1F.asm.)
	ld a, [wSavedCoordIndex]
	cp 1 ; index of second, lower entry in Route22DefaultScript.Route22RivalBattleCoords
	jr nz, .exit_movement_2
	call .RivalExit1Script
